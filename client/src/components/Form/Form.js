...
const Form = ({ currentId, setCurrentId }) => {
  ...
  const paciente = useSelector((state) =>
    currentId
      ? state.pacientes.pacientes.find((pac) => pac._id === currentId)
      : null
  );
  ...
  const { vista, page, tags, anomesStr, fechaAte1, diagnosticos, practicas } =
    contextVolver;
  ...
  const handleSubmit = (e) => {
    ...
    if (!pacienteData.dni || pacienteData.dni === "") {
      ...
      if (currentId === 0) {
        ...
        dispatch(createPaciente({ ...pacienteData }, navigate));
        clear();
      } else {
        dispatch(
          updatePaciente(currentId, {
            ...pacienteData,
    ...
    <Paper className={classes.paper} elevation={6}>
      ...
      >
        <Typography variant="h6">
          {currentId ? `Editando "${paciente?.dni}"` : "Crear un Paciente"}
        </Typography>
        <TextField
          size="small"
          name="dni"
        ...
        >
          ...
          <TextField
            ...
            onChange={(e) =>
              setPacienteData({ ...pacienteData, nombres: e.target.value })
            }
          />
        </Stack>
