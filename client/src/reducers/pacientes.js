...
const pacientesReducer = (
  state = { isLoading: true, pacientes: [] },
  action
) => {
  switch (action.type) {
    ...
    case CREATE_PAC:
      return {
        ...state,
        pacientes: !action.payload.dniDuplicado
          ? [...state.pacientes, action.payload.data]
          : state.pacientes,
        dniDuplicado: action.payload.dniDuplicado,
      };
      ...
      if (!action.payload.error) {
        return {
          ...
          pacientes: state.pacientes.map((paciente) =>
            paciente._id === action.payload.data._id
              ? action.payload.data
              : paciente
          ),
